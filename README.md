...
이 프로젝트는 학습 목적이지만, 보안 베스트 프랙티스를 따르고 있습니다.

#### ✅ 안전하게 구현된 부분

| 항목 | 구현 방법 | 위치 |
|-----|----------|------|
| **계정 정보 저장** | `flutter_secure_storage` 암호화 저장 | `lib/data/services/api_client.dart` |
| **비밀번호 전송** | AES-256-CBC 암호화 후 전송 | `lib/data/services/korail_crypto.dart` |
| **세션 관리** | 쿠키 기반, 메모리에만 저장 | `lib/data/services/korail_api.dart` |
| **환경 변수** | `.env` 파일 `.gitignore`에 포함 | `.gitignore` |
| **API 키** | 서버에서 동적으로 받아옴 | `korail_crypto.dart` |
...
1. **코드에 계정 정보 하드코딩 금지**
   ```dart
   // ❌ 절대 금지
   final id = "my_korail_id";
   final pw = "my_password";
   
   // ✅ SecureStorage 사용
   final id = await secureStorage.read(key: 'korail_id');
   ```

2. **API 키나 시크릿 Git 커밋 금지**
   ```bash
   # .gitignore에 반드시 포함
   .env
   *.key
...
3. **로그에 민감 정보 출력 금지**
   ...
   // ✅ 마스킹 처리
   print('Login attempt for user: ${id.substring(0, 3)}***');
   ```
...
- [ ] `.gitignore`에 `.env`, `*.key`, `*secret*` 포함
- [ ] 계정 정보는 `SecureStorage` 사용
- [ ] API 키는 하드코딩하지 않음
- [ ] 민감 정보는 로그에 출력하지 않음
- [ ] `git log`에 민감 정보 없는지 확인
- [ ] Firebase 설정 파일 `.gitignore`에 포함
- [ ] Release 빌드 전 디버그 코드 제거
